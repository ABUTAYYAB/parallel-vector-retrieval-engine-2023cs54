...
int main(int argc, char **argv) {
    ...
    MPI_Scatter(
        global_database.data(), local_documents * DIMENSIONS, MPI_FLOAT,
    ...
    for (int i = 0; i < local_documents; ++i) {
        ...
        // global document ID = rank offset + local index
    ...
    MPI_Gather(
        local_top_ids.data(),    TOP_K, MPI_INT,
        gathered_ids.data(),     TOP_K, MPI_INT,
        0, MPI_COMM_WORLD
    );
    MPI_Gather(
        local_top_scores.data(), TOP_K, MPI_FLOAT,
        gathered_scores.data(),  TOP_K, MPI_FLOAT,
        0, MPI_COMM_WORLD
    );
    ...
    if (world_rank == 0) {
        ...
        partial_sort(
            global_candidates.begin(),
            global_candidates.begin() + TOP_K,
        ...
        cout << "MPI," << world_size << ",1," << total_documents << ","
             << DIMENSIONS << ","
             << fixed << setprecision(5) << comm_time << ","
             << comp_time << ","
